/// @description Configuraçãoes inicial do animal

almaColetada = false; // Variável que diz se a alma do animal já foi coletada pelo jogador 
espacamentoDeIcone = 0; // Define o espaçamento dos sprites desenhados no topo da tela

#region //Frases desmotivacionais
global.frases = ["Lucifer não fez nada de errado.",
				"Baphomet reina <3.",
				"Gosto de sangue.",
				"Nada de bom pode vir daqui.",
				"Nem tente.",
				"Você é um anjo? Lucifer também era.",
				"Todo nós morremos sozinhos.",
				"Sua vida está um merda? Não há nada que não possa piorar.",
				"Eu pensei que era um fodido, até te conhecer.",
				"Você não significa nada no esquema do universo.",
				"Você não é especial.",
				"Foi tudo culpa sua",
				"Ele vai chegar, ele está aqui, ele já está entre nós"];
#endregion